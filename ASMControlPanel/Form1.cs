    ...
    public partial class Form1 : Form
    {
        ...
        private string GetSpecificProjectFolderName(string dir)
        {
            ...
            try
            {
                ...
                FileInfo[] fi = new DirectoryInfo(dir).GetFiles("Project*");
                if (fi.Length > 0)
                {
        ...
        private void PrepareProjectSelection()
        {
            ...
            try{
                ...
                string[] projects = Directory.GetDirectories(@"c:\", "Project*", SearchOption.TopDirectoryOnly);
        ...
        private void GetPlatformInfo(string projDir){
            ...
            if (projDir == DEFAULT_PROJECT_DIR){
                ...
                if (RUNNING_PROJECT == @"c:\"){ //New one does not have specific project folder file created yet.
        ...
        private string GenerateStandardFormatProjectFolderName()
        {
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                specificFoler = "Project_" + SOFTREV + "_" + config;
            }
            file.Close();

            //Thread.Sleep(1000);
            logger("DEFAULT_PROJECT_DIR: ", DEFAULT_PROJECT_DIR);
            logger("specificFoler: ", specificFoler);

            var myFile = File.CreateText(Path.Combine(DEFAULT_PROJECT_DIR, specificFoler));
            myFile.Close(); //Must close it; otherwise, might cause trouble for other file/folder changes at later time    
        ...
        void PrepareRCSimulation(bool option){ //true: enable PM simulation, false: disable 
            ...
            if (!option){
                ...
                ModifyMCIFSimFile("=True", "=False"); //enable option selected
        ...
        private void RCSIMSelection(bool selection){ //false : unchecked, true: checked
            ...
            switch (SELECTED_RC){
                case "PMC1": checkBox1.Checked = selection; break;
                case "PMC2": checkBox2.Checked = selection; break;
                case "PMC3": checkBox3.Checked = selection; break;
                case "PMC4": checkBox4.Checked = selection; break;
            }
        }
